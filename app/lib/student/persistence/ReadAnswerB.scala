...
/**
 * ReadAnswerB Information
 */
case class ReadAnswerBRepository[P <: JdbcProfile]()(implicit val driver: P)
  ...
  // --[ Methods ]--------------------------------------------------------------
  /**
   * Get ReadAnswerB data
   */
  def get(id: Id): Future[Option[EntityEmbeddedId]] =
    RunDBAction(ReadAnswerBTable, "slave") { _
      .filter(_.id === id)
      .result.headOption
    }
  ...
  // --[ Methods ]--------------------------------------------------------------
  /**
   * Add ReadAnswerB data
   */
  def add(data: EntityWithNoId): Future[Id] =
    RunDBAction(ReadAnswerBTable) { slick =>
      slick returning slick.map(_.id) += data.v
    }

  /**
   * Update ReadAnswerB data
   */
  ...
  /**
   * Eliminate specified ReadAnswerB data
   */
